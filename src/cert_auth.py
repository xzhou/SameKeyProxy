...
def save_cert_key_file(real_cert, fake_key, fake_cert):
    """Save the private key and cert for later analysis."""
    print 'entering save_cert_key_file'
    
    domain = real_cert.get_subject().CN
    key_file = open('certs/' + domain + '.t.key', 'w')
    key_file.write(fake_key.as_pem(None))
    key_file.close()
    
    cert_file = open('certs/' + domain + '.t.cert', 'w')
    cert_file.write(fake_cert.as_text() + '\n' + fake_cert.as_pem())
    cert_file.close()
    print 'exit saving key file'
...
class CertAuth(object):
    ...
    def generate_same_key(self):
        """
            This gives the same RSA key pair for all connections so we
            can easily decrypt them in wireshark. 
        """
        return M2Crypto.RSA.load_key("ca/start.key");
    ...
    def cert (self, peer_sub, peer_iss, peer_not_after, peer_not_before,
        ...
        """
            ...
            @return cert and key
        """
        # Modified by xzhou to reuse private key for easy wireshark decryption
        #key = self.generate_rsa_key()
        key = self.generate_same_key()
    ...
    def get_fake_cert_and_key_filename(self, real_cert):
        '''
        Create a fake cert from the real cert. 
